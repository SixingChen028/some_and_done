...
best_actions = np.zeros((len(n_depth_set), n_world))
expected_accuracies = np.zeros((len(n_depth_set), len(n_sample_set), n_world))
...
for i_depth, n_depth in enumerate(n_depth_set):
    ...
    for i_world in range(n_world):
        ...
        best_actions[i_depth, i_world] = best_action
        ...
        for i_sample, n_sample in enumerate(n_sample_set):
            ...
            expected_accuracies[i_depth, i_sample, i_world] = exacc
            

    for i_sample, n_sample in enumerate(n_sample_set):
        exrew = expected_rewards[i_depth, i_sample, :]
        print(
            'depth:', n_depth, '|',
            'sample num:', n_sample, '|',
...
with open('data/data_simulation/data_depth.p', 'wb') as f:
    pickle.dump(data, f)