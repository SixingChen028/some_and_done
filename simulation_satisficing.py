...
choices_satisficing = np.zeros((2, len(n_depth_set), len(noise_set), n_world)) # infinite samples vs. finite (5) samples
...
for i_depth, n_depth in enumerate(n_depth_set):
    ...
    for i_noise, noise in enumerate(noise_set):
        ...
        for i_world in range(n_world):
            ...
            mus[:n_half] += 10 # big reward at the start for one depth-1 node
            mus[0] -= 15 # big panelty at the end for one leaf node
            ...
            # record
            choices_satisficing[0, i_depth, i_noise, i_world] = np.argmax(true_action_rewards) # get choices from real values
            choices_satisficing[1, i_depth, i_noise, i_world] = np.mean(choices) # get choices from finite samples
...
data = {
    ...
    'n_test': n_test,
    'sig_r': sig_r,
    'choices_satisficing': choices_satisficing,
}

with open('data/data_simulation/data_satisficing.p', 'wb') as f:
    pickle.dump(data, f)